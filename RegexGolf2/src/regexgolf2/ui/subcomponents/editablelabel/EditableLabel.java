 ...
 * Text of the Label can be edited. Can show a small icon to indicate that
 * editing is possible, this has to be set with the
 * {@link #editIconAppearsProperty()}. Editing can be disabled. EditMode can be
 * entered and exited.
 */
public class EditableLabel
{
	...
	private void initLayout()
	{
		...
		_root.prefWidthProperty().bind(
				_textField.prefWidthProperty().add(_textField.heightProperty()));
	...
	/**
	 * Initializes various Listeners: - If the Enter-Key is pressed in the
	 * TextField, editing mode is exited. - If the TextField loses focus,
	 * editing mode is exited. - If editing gets disabled, the editing mode is
	 * exited. - If edit mode is entered, the TextField requests focus.
	 */
	private void initListeners()
	{
		// TextField enter key pressed -> exit edit mode
		_textField.setOnAction(e -> exitEditMode());

		// TextField focus lost -> exit edit mode
		_textField.focusedProperty().addListener((o, oV, focused) ->
		{
			if (!focused)
				exitEditMode();
		});

		_editable.addListener((o, oV, editable) ->
		{
			if (!editable && isEditMode())
				exitEditMode();
		});

		_editMode.addListener((o, oV, activated) ->
		{
			if (activated)
	...
	/**
	 * Tries to enter edit mode. if editing is disabled, this will return false,
	...
	// --------------------------- editIconVisible ---------------------
	/**
	 * The edit icon will only be visible if three criteria are met: 1. Editing
	 * is enabled. 2. Edit mode is currently disabled. 3. This Property is set
	 * to true. Usually, this is bound to a mouse hover Property to make the
	 * icon show up if the mouse hovers a specific area.
	 */
	@Ensures("result != null")
